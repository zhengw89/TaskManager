    ...
    public class Database : IDisposable
    {
        ...
        public void BeginTransaction(IsolationLevel? il = null)
        {
            ...
            if (_transactionDepth == 1)
            {
                ...
                _transaction =
                    il == null ? _sharedConnection.BeginTransaction() : _sharedConnection.BeginTransaction(il.Value);
        ...
        public bool InsertMutilRecord<T>(IEnumerable<T> collection)
        {
            bool result = true;
            try
            {
                ...
                using (var cmd = CreateCommand(_sharedConnection, ""))
                {
                    ...
                    else if (_dbType == DBType.Oracle || _dbType == DBType.SqlServer)
                    {

                        var index = 0;
                        string kw = " UNION ALL " + (_dbType == DBType.Oracle ? "DUAL" : "");
                        List<string> values = new List<string>();
                        foreach (var poco in collection)
                        {
                            foreach (var i in pd.Columns)
                            ...
                            pocoValues.Add("SELECT " + string.Join(",", values.ToArray()) + kw);
                            values.Clear();
                        }

                        sql = string.Format("INSERT INTO {0} ({1}) {2}", tableName, cols, Regex.Replace(string.Join("", pocoValues), kw + "$", ""));
